...
print os.path.join(LISPMON_TRACES, "{0}_{1}_{2}".format(now.year, now.month, now.day), 'locator_number.csv')


rloc_num_list = []
with open(os.path.join(LISPMON_TRACES, "{0}_{1}_{2}".format(now.year, now.month, now.day), 'locator_number.csv'), 'wb') as eid_num_file:
    spamwriter = csv.writer(eid_num_file, dialect='excel', delimiter=';')
    # Write the first line, i.e., title
    spamwriter.writerow(['Date', 'RLOC number', 'RLOCs'])
            ...
            for day in day_list:
                ...
                try:
                    ...
                    for line in html.split('\n'):
                        ...
                        if line.startswith('>'):
                            rloc_list.append(line.split(' ')[1])

                    eid_counter = len(list(set(rloc_list)))
                    rloc_num_list.append(eid_counter)
                    print '{0}-{1}-{2}:'.format(year,month,day), eid_counter

                    row = ['{0}-{1}-{2}'.format(year,month,day), eid_counter]
                    row.extend(list(set(rloc_list)))
