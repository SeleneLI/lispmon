...
plt.plot(x, eid_num_list)
plt.xlabel(r"\textrm{2012/01/07--2016/05/15}", font)
plt.ylabel(r"\textrm{EID number}", font)
plt.xlim(min(x),max(x))
plt.xticks(fontsize=30, fontname="Times New Roman")
plt.yticks(fontsize=30, fontname="Times New Roman")

plt.savefig(os.path.join(LISPMON_FIGURES_AND_TABLES, "{0}_{1}_{2}".format(now.year, now.month, now.day), 'eid_number.eps'), dpi=300, transparent=True)
plt.show()